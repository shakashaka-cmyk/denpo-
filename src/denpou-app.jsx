...
const DenpouApp = () => {
  ...
  const submitAnswer = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/games/${gameID}/answer`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Player-ID': playerID,
      ...
      const currentRoundData = updatedGame.rounds[currentRound];
      if (currentRoundData?.correctAnswer) {
        setAnswerFeedback('✅ 正解！');
      } else {
        setAnswerFeedback('❌ ハズレ');
      }
    } catch (err) {
      setErrorMsg('失敗しました');
    }
  ...
  if (appState === 'game' && game && game.rounds && game.rounds.length > 0) {
    // 親の場合、自分が親のラウンドを表示
    const isPlayerParent = game.rounds.some(r => r.parentId === playerID);
    
    // 実行中のラウンドを全員が見る
    const activeRound = game.rounds.find(r => r.status === 'hint_phase');
    const displayRound = activeRound ? game.rounds.indexOf(activeRound) : currentRound;

    const currentRoundData = game.rounds[displayRound];
